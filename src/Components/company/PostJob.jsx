...
const PostJob = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ use proxy and DON'T set Content-Type (fetch sets multipart for FormData)
      const response = await fetch("/api/jobs/", {
        method: "POST",
        body: formData,
      ...
      console.log("Post /api/jobs/ raw response:", raw);
      let result;
      try {
      ...
      if (!response.ok) {
        throw new Error(
          result?.detail ||
          result?.message ||
          (typeof result === "string" ? result : "Failed to post job")
        );
      }
      ...
      >
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          ...
          <button
            ...
            onMouseDown={(e) =>
              !isLoading && (e.target.style.transform = "scale(0.98)")
            }
            onMouseUp={(e) =>
        ...
        {jobs.length > 0 && (
          <div style={{ marginTop: "3rem" }}>
            <h3 style={{ marginBottom: "1rem", color: "#2c3e50", fontSize: "1.5rem" }}>
              Posted Jobs
            </h3>
              ...
              <tbody>
                {jobs.map((job) => (
                  <tr key={job.id}>
                    ...
                    <td style={{ padding: "0.75rem", border: "1px solid #eee" }}>
                      {/* backend returns "/media/job_pdfs/..." which works with proxy */}
                      <a
                        href={job.pdf_file}
                        target="_blank"
